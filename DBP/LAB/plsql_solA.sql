...
IS
  ...
  emp_rec C1%ROWTYPE;	-- 커서를 위한 레코드 변수 선언
...
BEGIN
  ...
  /* Cursor와 loop를 사용하여 검색 결과를 하나씩 출력하는 코드 */
  OPEN C1;
  LOOP 				
    FETCH C1 INTO emp_rec; 	
    EXIT WHEN C1%NOTFOUND;	   
    dbms_output.put_line(RPAD(emp_rec.empno,10) || RPAD(emp_rec.ename,10) || 
                    RPAD(emp_rec.job,10) || LPAD(emp_rec.sal,10) || LPAD(emp_rec.comm,10));
  END LOOP;
  CLOSE C1;		
  dbms_output.put_line('------------------------------------');
  ...
  UPDATE emp1  
  SET comm = NVL(comm,0) + comm_inc -- comm이 null일 경우 0으로 간주하고 계산
  WHERE empno = new_mgr; 
...
set serveroutput on
var return NUMBER
execute :return := lab4a2('ACCOUNTING', 7934 , 1010);
print return
